...
async def auto_detect_features(
    ...
    async def event_stream():
        try:
            # ── Step 1: Analyze room layout ───────────────────────────────
            yield _sse({"step": 1, "total": 6, "message": "Analyzing your room layout..."})
            pil_image = Image.open(io.BytesIO(image_data)).convert("RGB")
            image_np = np.array(pil_image)
            h, w = image_np.shape[:2]

            # ── Step 2: Process texture ───────────────────────────────────
            yield _sse({"step": 2, "total": 6, "message": "Processing Travertine Stone texture..."})
            await asyncio.sleep(0.2)

            # ── Step 3: Apply quality settings ────────────────────────────
            yield _sse({"step": 3, "total": 6, "message": "Applying Standard quality settings..."})
            await asyncio.sleep(0.2)

            # ── Step 4: AI segmentation ───────────────────────────────────
            yield _sse({"step": 4, "total": 6, "message": "AI is generating magic..."})
            floor_mask, wall_mask = await asyncio.to_thread(predictor.predict, image_np)

            # ── Step 5: Lighting recovery ─────────────────────────────────
            yield _sse({"step": 5, "total": 6, "message": "Adding realistic lighting..."})
            await asyncio.sleep(0.2)

            # ── Step 6: Post-processing ───────────────────────────────────
            yield _sse({"step": 6, "total": 6, "message": "Finalizing your transformation..."})
            ...
            # ── Build compressed binary payload ───────────────────────────
