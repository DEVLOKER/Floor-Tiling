...
.PARAMETER LicenseUsbPath
    Path to mount as /license inside the container (default: env LICENSE_USB_PATH or D:\license).

.EXAMPLE
    .\run_docker.ps1 -Tag 1.0.0
    .\run_docker.ps1 -Tag latest -Port 9000
    .\run_docker.ps1 -Tag 1.0.0 -LicenseUsbPath "E:\license"
#>
param(
    [Parameter(Mandatory)]
    [string]$Tag,
...
if (-not $LicenseUsbPath) {
    $LicenseUsbPath = if ($env:LICENSE_USB_PATH) { $env:LICENSE_USB_PATH } else { "D:\license" }
}
...
docker run `
    ...
    -p "${Port}:8000" `
    -e "LICENSE_USB_PATH=$LicenseUsbPath" `
    "${ImageName}:${Tag}"
