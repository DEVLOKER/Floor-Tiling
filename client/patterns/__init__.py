...
def pattern_versailles(u: np.ndarray, v: np.ndarray, grout_h_frac, grout_v_frac, aspect_ratio: float=1.0, uv_step_u=None, uv_step_v=None, grout_thickness_v: int=1, grout_thickness_h: int=1, du_dx=None, du_dy=None, dv_dx=None, dv_dy=None, **_) -> tuple:
    ...
    is_second = ~in_center
    return (is_second, alpha)
PATTERN_FUNCTIONS = {'grid': pattern_grid, 'brick': pattern_brick, 'herringbone': pattern_herringbone, 'checkerboard': pattern_checkerboard, 'chevron': pattern_chevron, 'basketweave': pattern_basketweave, 'versailles': pattern_versailles}

def get_pattern(pattern_name: str):
    return PATTERN_FUNCTIONS.get(pattern_name, pattern_grid)
__all__ = ['pattern_grid', 'pattern_brick', 'pattern_herringbone', 'pattern_checkerboard', 'pattern_chevron', 'pattern_basketweave', 'pattern_versailles', 'PATTERN_FUNCTIONS', 'get_pattern']
